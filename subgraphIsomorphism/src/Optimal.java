...
public class Optimal {
	...
	public boolean isSubgraph() {
		...
		for(int i = 0; i < H.size ; i++) {
			if(G.verticesSorted[i].degree > H.largest.degree) {
				startpoints.add(G.verticesSorted[i]);
			}
		}
		if(!startpoints.get(0).isPromising(H.verticesSorted[0])) {
		...
		while(!startpoints.isEmpty()) {
			...
			trasub.add(H.verticesSorted[0].index);
			trabig.add(startpoints.get(0).index);

			Checknode check = new Checknode(startpoints.get(0),H.verticesSorted[0],trasub,trabig);
